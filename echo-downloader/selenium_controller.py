...
def retrieve_cookie(driver: WebDriver, site: str) -> Optional[list[dict[str, Any]]]:
   try:
        driver.get(site)
        time.sleep(0.5)  # Brief wait for cookies to settle
        return driver.get_cookies()  # type: ignore
   except Exception as e:
        logger.error(f"Failed to retrieve cookies from {site}: {e}")
        return None

def retrieve_ms_auth_cookies(driver: WebDriver) -> list[dict[str, Any]]:
    """Retrieve cookies from all relevant auth domains for SSO."""
    all_cookies: list[dict[str, Any]] = []
    seen = set()  # Track unique cookies by (name, domain)
    ...
    for url in auth_urls:
        cookies = retrieve_cookie(driver, url)
    ...
    logger.debug(f"Retrieved {len(all_cookies)} total auth cookies")
    return all_cookies
